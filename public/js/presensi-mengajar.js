...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const inputMataPelajaranId = document.getElementById(
        "inputMataPelajaranId",
    );
    ...
    const filterTanggalSelesai = document.getElementById(
    ...
    const applyWaktuKbm = (waktuRange, hari, durasiJp) => {
        if (wrapWaktuKbm && textWaktuKbm) {
            if (waktuRange) {
                ...
                if (badgeHariJadwal)
                    badgeHariJadwal.textContent = hari || "Jadwal";
    ...
    const checkStatusRadio = () => {
        ...
        if (wrapGuruPengganti) {
            wrapGuruPengganti.style.display =
                checkedVal === "D" ? "block" : "none";
        }
    };
    ...
    if (selectJadwalKbm) {
        selectJadwalKbm.addEventListener("change", () => {
            ...
            if (inputRombel && opt.dataset.rombelId)
                inputRombel.value = opt.dataset.rombelId;
            if (inputMapel && opt.dataset.mapel)
                inputMapel.value = opt.dataset.mapel;
            if (inputPembelajaranId)
                inputPembelajaranId.value = opt.dataset.pembelajaranId || "";
            if (inputMataPelajaranId)
                inputMataPelajaranId.value = opt.dataset.mapelId || "";
            ...
            applyWaktuKbm(
                opt.dataset.jamWaktu,
                opt.dataset.hari,
                opt.dataset.durasiJp,
            );
        });
    }
    ...
    const openCreateModal = () => {
        ...
        const radioH = document.querySelector(
            'input[name="status"][value="H"]',
        );
    ...
    const infoKalenderBox = document.getElementById(
        "infoKalenderTanggalPresensi",
    );
    const checkKalenderDate = async (dateVal) => {
        if (!infoKalenderBox || !dateVal) return;
        try {
            const res = await fetch(
                `/dashboard/presensi-mengajar/cek-kalender?tanggal=${encodeURIComponent(dateVal)}`,
                {
    ...
    document.querySelectorAll(".btn-quick-checkin").forEach((btn) => {
        btn.addEventListener("click", () => {
            ...
            if (selectJadwalKbm && d.jadwalId)
                selectJadwalKbm.value = d.jadwalId;
            if (inputRombel && d.rombelId) inputRombel.value = d.rombelId;
            if (inputMapel && d.mapel) inputMapel.value = d.mapel;
            if (inputPembelajaranId)
                inputPembelajaranId.value = d.pembelajaranId || "";
            if (inputMataPelajaranId)
                inputMataPelajaranId.value = d.mapelId || "";
            if (inputJamKeMulai && d.jamMulai)
    ...
    document
        .querySelectorAll(".btn-close-modal, .btn-close-detail")
        .forEach((btn) => {
    ...
    window.refreshLiveTable = (url = null) => {
        ...
        if (!targetUrl) {
            ...
            if (liveSearchInput?.value)
                params.set("q", liveSearchInput.value.trim());
            if (filterTanggalMulai?.value)
                params.set("tanggal_mulai", filterTanggalMulai.value);
            if (filterTanggalSelesai?.value)
                params.set("tanggal_selesai", filterTanggalSelesai.value);
            if (filterRombel?.value)
                params.set("rombongan_belajar_id", filterRombel.value);
    ...
    if (formPresensi) {
        formPresensi.addEventListener("submit", (e) => {
            ...
            const url = isEdit
                ? `/dashboard/presensi-mengajar/${presensiId.value}`
                : "/dashboard/presensi-mengajar";
    ...
    document.addEventListener("click", (e) => {
        ...
        fetch(`/dashboard/presensi-mengajar/${id}`, {
            ...
            .then((res) => {
                ...
                if (inputJadwalKbmId)
                    inputJadwalKbmId.value = d.jadwal_kbm_id || "";
                if (selectJadwalKbm)
                    selectJadwalKbm.value = d.jadwal_kbm_id || "";
                ...
                if (inputJamMasuk)
                    inputJamMasuk.value = d.jam_masuk
                        ? d.jam_masuk.substring(0, 5)
                        : "";
                if (inputJamKeluar)
                    inputJamKeluar.value = d.jam_keluar
                        ? d.jam_keluar.substring(0, 5)
                        : "";
                if (inputGuruPengganti)
                    inputGuruPengganti.value = d.nama_guru_pengganti ?? "";
                if (inputKeterangan) inputKeterangan.value = d.keterangan ?? "";

                if (d.jam_masuk && d.jam_keluar) {
                    applyWaktuKbm(
                        `${d.jam_masuk.substring(0, 5)} - ${d.jam_keluar.substring(0, 5)}`,
                        d.hari,
                        d.total_jp,
                    );
                } else {
