@php
    $user = session('user', ['name' => 'Pengguna', 'role' => 'siswa']);
...
<aside class="dash-sidebar" id="dashSidebar">
    ...
    <div class="dash-sidebar-header">
        <a href="{{ route('dashboard.' . $role) }}" class="brand">
            <img id="dashLogo" 
                 ...
                 data-dark="{{ asset('img/logo-dark.png') }}" 
                 data-light="{{ asset('img/logo-light.png') }}" 
                 alt="SAE Logo">
        </a>
    </div>
    ...
    <div class="dash-sidebar-user">
        ...
        <div class="dash-user-info">
            <div class="dash-user-name" title="{{ $user['name'] }}">{{ $user['name'] }}</div>
    ...
    <div class="dash-sidebar-nav">
        ...
        @if($role === 'admin')
            <a href="{{ route('dashboard.admin') }}" class="dash-nav-link {{ request()->routeIs('dashboard.admin') ? 'active' : '' }}">
            ...
            <a href="#" class="dash-nav-link">
                <i class="fas fa-rotate"></i> <span>Sinkronisasi Dapodik</span>
            </a>
        ...
        @elseif($role === 'guru')
            <a href="{{ route('dashboard.guru') }}" class="dash-nav-link {{ request()->routeIs('dashboard.guru') ? 'active' : '' }}">
        ...
        @elseif($role === 'siswa')
            <a href="{{ route('dashboard.siswa') }}" class="dash-nav-link {{ request()->routeIs('dashboard.siswa') ? 'active' : '' }}">
    ...
    <div class="dash-sidebar-footer">
        ...
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="dash-nav-link" style="width: 100%; border: none; background: rgba(239, 68, 68, 0.1); color: #ef4444; cursor: pointer; text-align: left;">
