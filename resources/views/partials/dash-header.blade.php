<header class="dash-header">
    ...
    <div class="dash-header-left">
        ...
        @php
            $headerUser = session('user');
            $headerRole = is_array($headerUser) ? ($headerUser['role'] ?? 'peserta_didik') : ($headerUser->role ?? 'peserta_didik');
        @endphp
        @if ($headerRole === 'admin')
            <div class="dash-custom-dropdown" style="margin-left: 10px;">
                <button type="button" class="custom-dropdown-trigger"
                    style="padding: 4px 10px; font-size: 0.74rem; border-radius: 8px; background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.28); color: var(--text-color); height: 32px;"
                    aria-haspopup="listbox" aria-expanded="false" title="Pindah Mode Dashboard (Superadmin)">
                    <div class="custom-dropdown-trigger-label" style="display: flex; align-items: center; gap: 6px;">
                        <i class="fas fa-crown text-warning" style="font-size: 0.75rem;"></i>
                        <span style="font-weight: 700; font-size: 0.76rem;">
                            {{ request()->routeIs('dashboard.guru') ? 'Dashboard Guru' : (request()->routeIs('dashboard.tendik*') ? 'Dashboard Tendik' : (request()->routeIs('dashboard.peserta*') ? 'Dashboard Siswa' : 'Dashboard Admin')) }}
                        </span>
                    </div>
                    <i class="fas fa-chevron-down custom-dropdown-arrow" style="font-size: 0.65rem; margin-left: 6px;"></i>
                </button>
                <div class="custom-dropdown-menu" role="listbox" style="min-width: 190px;">
                    <a href="{{ route('dashboard.admin') }}"
                        class="custom-dropdown-item {{ request()->routeIs('dashboard.admin') ? 'active' : '' }}"
                        style="text-decoration: none; padding: 8px 12px; font-size: 0.8rem;">
                        <div class="dropdown-item-left" style="display: flex; align-items: center; gap: 8px;">
                            <i class="fas fa-user-shield text-primary" style="width: 16px; text-align: center;"></i>
                            <span>Dashboard Admin</span>
                        </div>
                        @if (request()->routeIs('dashboard.admin'))
                            <span class="badge badge-primary badge-sm"><i class="fas fa-check"></i></span>
                        @endif
                    </a>
                    <a href="{{ route('dashboard.guru') }}"
                        class="custom-dropdown-item {{ request()->routeIs('dashboard.guru') ? 'active' : '' }}"
                        style="text-decoration: none; padding: 8px 12px; font-size: 0.8rem;">
                        <div class="dropdown-item-left" style="display: flex; align-items: center; gap: 8px;">
                            <i class="fas fa-chalkboard-user text-success" style="width: 16px; text-align: center;"></i>
                    ...
                    <a href="{{ route('dashboard.tendik') }}"
                        class="custom-dropdown-item {{ request()->routeIs('dashboard.tendik*') ? 'active' : '' }}"
                        style="text-decoration: none; padding: 8px 12px; font-size: 0.8rem;">
                        <div class="dropdown-item-left" style="display: flex; align-items: center; gap: 8px;">
                            <i class="fas fa-id-badge text-info" style="width: 16px; text-align: center;"></i>
                            <span>Dashboard Tendik</span>
                        </div>
                        @if (request()->routeIs('dashboard.tendik*'))
                            <span class="badge badge-primary badge-sm"><i class="fas fa-check"></i></span>
                        @endif
                    </a>
                    <a href="{{ route('dashboard.peserta-didik') }}"
                        class="custom-dropdown-item {{ request()->routeIs('dashboard.peserta*') ? 'active' : '' }}"
                        style="text-decoration: none; padding: 8px 12px; font-size: 0.8rem;">
                        <div class="dropdown-item-left" style="display: flex; align-items: center; gap: 8px;">
                            <i class="fas fa-user-graduate text-warning" style="width: 16px; text-align: center;"></i>
